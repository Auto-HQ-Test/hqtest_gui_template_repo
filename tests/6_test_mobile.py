...
@function_logging
async def check_mobile_KR(settings, test_logger):
    pass


@function_logging
async def check_mobile_EN(settings, test_logger):
    pass
...
async def test_browser(settings, test_logger):
    ...
    if not settings.should_run_module(current_module):  # 설정에 따라 모듈 실행 여부 확인
        ...
        # 한국어 배너 확인
        await check_mobile_KR(settings, test_logger)
        
        # 영어 배너 확인
        await check_mobile_EN(settings, test_logger)
